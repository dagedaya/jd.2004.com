...
function scrollFloorView(){
	...
	if(ling > 1200) {
		$("#floor-login").show(); //如果滚动的距离大于1200，滚动框显示出来
	}
	if(1200 < ling && ling < 1600) { //第一层的数字隐藏，文字显示，其他兄弟元素的li数字显示，文字隐藏
		$("#floor-login ul li").eq(0).find(".num").hide().siblings(".word").css("display", "block");
		$("#floor-login ul li").eq(0).siblings("li").find(".num").css("display", "block").siblings(".word").hide();
	} else if(ling < 1800) {
		$("#floor-login ul li").eq(1).find(".num").hide().siblings(".word").css("display", "block");
		$("#floor-login ul li").eq(1).siblings("li").find(".num").css("display", "block").siblings(".word").hide();
	}
...
function resizeFloorView(){
	if($(document.body).width() > 1200){
			...
			$("#floor-login").css("left", ele_width + "px");
		}
		else{
			$("#floor-login").css("left", "6px");
		}
}
$(function() {
	resizeFloorView();
